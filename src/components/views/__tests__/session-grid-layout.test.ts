...
describe("reconcileLayout batch placement", () => {
  const base = {
    rows: [{ cells: ["a"], colSizes: [1] }],
    rowSizes: [1],
  };

  it("opens the block in a fresh bottom row, never topping up a partial row", () => {
    const next = reconcileLayout(base, ["a", "n1"], [], {
      ...NO_PLACEMENT,
      anchor: "a",
      maxPerRow: 2,
      batch: { first: true },
    });
    expect(rowsOf(next)).toEqual([["a"], ["n1"]]);
  });

  it("continues the block by filling the bottom row, wrapping at the cap", () => {
    let layout = reconcileLayout(base, ["a", "n1"], [], {
      ...NO_PLACEMENT,
      maxPerRow: 2,
      batch: { first: true },
    });
    layout = reconcileLayout(layout, ["a", "n1", "n2", "n3"], [], {
      ...NO_PLACEMENT,
      maxPerRow: 2,
      batch: { first: false },
    });
    layout = reconcileLayout(layout, ["a", "n1", "n2", "n3", "n4"], [], {
      ...NO_PLACEMENT,
      maxPerRow: 2,
      batch: { first: false },
    });
    // A 2×2 block under the pre-existing partial row.
    expect(rowsOf(layout)).toEqual([["a"], ["n1", "n2"], ["n3", "n4"]]);
  });
});
